...
```
dist/SecureMailScopeCaptureAgent-1.0.2-Setup.exe
```
...
### Installer Handling
The installer (`SecureMailScopeCaptureAgent-1.0.2-Setup.exe`) implements safe, dual-mode prerequisite handling:
...
5. Compile `inno_setup.iss` into `dist\SecureMailScopeCaptureAgent-1.0.2-Setup.exe`.
